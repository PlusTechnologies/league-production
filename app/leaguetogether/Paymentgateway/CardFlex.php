...
//        $this->blueFlex($param,'Sale');
    ...
    public function flex($param, $type){
        ...
        $playerParticipant = Participant::where('player_id', $item->player_id)->whereRaw("BINARY event_id  = '$item->id'")->first();
    		//$playerParticipant = Participant::where('event_id', $item->id)->where('player_id','=',$item->player_id)->first();

    		if((!empty($playerMember->status) || !empty($playerParticipant->status)) && !$item->autopay){
    			return  array("response"=>2, "responsetext"=>"The selected player is already registered $item->autopay $playerMember $playerParticipant ");
    		}
		};

		return  array("response"=>2, "responsetext"=>"True payment $playerMember $playerParticipant");
